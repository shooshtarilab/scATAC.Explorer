...
```{r}
res = queryATAC(tumour_type = 'leukemia', metadata_only = TRUE)[[1]]
```
```{r, echo=FALSE}
...
## Example: Accessing counts matrix
####### TODO: ASK ERIC ABOUT counts below and the data frame display
The counts matrix can be accessed using the counts() function, stored in a sparse matrix format by default:
```{r, eval=FALSE}
res = queryATAC(geo_accession = "GSE89362")
counts(res[[1]])[1:5,1:5]
```
```{r, echo=FALSE}
